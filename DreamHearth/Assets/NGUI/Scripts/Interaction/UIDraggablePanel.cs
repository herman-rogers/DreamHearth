...
public class UIDraggablePanel : IgnoreTimeScale
{
	...
	public GameObject activityScrollBar; //CUSTOM SCRIPT
	...
	/// <summary>
	/// Custom value used to effect the rate of decay in the scroll movement. - James Timms -06/08/2013.
	/// </summary>
	
	public float panelFriction;
	public float springFriction;
	
	public delegate void OnDragUpdate();
	public OnDragUpdate onDragUpdate;
	
//	
//	public float dampenStrengthModifier;
	...
	public void UpdateScrollbars (bool recalculateBounds)
	{
		...
		if (horizontalScrollBar != null || verticalScrollBar != null)
		{
			...
			if (verticalScrollBar != null && bmax.y > bmin.y)
			{
				...
				if (activityScrollBar == null){
				mIgnoreCallbacks = true;
				verticalScrollBar.barSize = 1f - (sum - 0.14f); //original is "sum" by itself
				verticalScrollBar.scrollValue = (sum > 0.001f) ? 1f - min / sum : 0f;
				mIgnoreCallbacks = false;
				}else{
					mIgnoreCallbacks = true;
					verticalScrollBar.barSize = 1f - (sum + 0.35f); //original is "sum" by itself
					verticalScrollBar.scrollValue = (sum > 0.001f) ? 1f - min / sum : 0f;
					mIgnoreCallbacks = false;					
				}
			}
	...
	public void Drag ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
		{
			...
			if (mPlane.Raycast(ray, out dist))
			{
				...
				if((offset != currentPos) && onDragUpdate != null)
					onDragUpdate();
	...
	void LateUpdate ()
	{
		//Check to see if any friction is 0.
		if(panelFriction <= 0.0f)
			panelFriction = 0.4f;
		if(springFriction <= 0.0f)
			springFriction = 0.4f;
		...
		if (mShouldMove && !mPressed)
		{
			...
			if (mMomentum.magnitude > 0.0001f)
			{
				...
				//Work out which friction is being used.
				if((!(sp == null)) &&
					sp.enabled)
					mFriction = springFriction;
				else
					mFriction = panelFriction;
				
				// Move the panel
				Vector3 offset = NGUIMath.SpringDampen(ref mMomentum, mFriction * 9f, delta);
				MoveAbsolute(offset);
						
				//Used for updating the drag effect while it is running.
				if(onDragUpdate != null) onDragUpdate();
			...
			else
			{
				mScroll = 0f;
				mMomentum = Vector3.zero;
				mFriction = panelFriction;//Restore panel friction once movement/spring is about done.
			}
		}
		else mScroll = 0f;
		
		// Dampen the momentum
		NGUIMath.SpringDampen(ref mMomentum, mFriction * 9f, delta);
	}
